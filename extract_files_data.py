...
VERSION = "BILAN CARBONE — Extraction finale V22 - lisible, HDQUALITE, anti-doublon corrigé"
...
ALLOW_WITH_BILAN_CARBONE = [
    "recap achat",
    "récap achat",
    "achat pour bilan carbone",
    "synthese donnees",
    "synthèse données",
    "donnees pour bilan carbone",
...
def is_non_source(filename: str) -> bool:
    """Retourne True seulement pour les fichiers qui ne sont pas des sources.

    Correction importante : un fichier peut contenir "bilan carbone" dans son nom
    tout en étant un fichier de collecte. On ignore donc les feuilles de résultat
    déjà calculé, mais on garde les feuilles de données brutes.
    """
    ...
    if any(k in f for k in already_calculated):
        return True
...
def scope_for_role(default_scope: str, role: str, designation: str) -> str:
    """Corrige les erreurs de classification quand le mapper se trompe de dossier.

    Exemple : une facture Orange classée en SCOPE_2 reste une charge externe
    de Scope 3, et une feuille EDF reste en Scope 2.
    """
    r = norm(role)
    d = norm(designation)
    ...
    if r == "edf" or d in {"electricite", "électricité"}:
        return "SCOPE_2"

    if r in {"propane", "combustible_direct", "carburant_scope1"}:
        return "SCOPE_1"
...
def make_item(
    ...
    unite: str,
    fiabilite: str = "haute",
    justification: str = "extraction directe",
) -> dict:
    final_scope = scope_for_role(scope, role, designation)
...
# ---------------------------------------------------------------------------
# Politique d'extraction monétaire générique
# ---------------------------------------------------------------------------
# Objectif : éviter de sous-estimer le Scope 3.
# Les montants en euros utiles sont conservés et classés en estimation variable.
# Les postes très agrégés ne sont plus bloqués : ils sont marqués avec une
# incertitude forte. Les seuls rejets doivent concerner les fichiers non sources,
# les valeurs incohérentes ou les données inutiles pour un bilan carbone.
#
# Les facteurs ci-dessous ne calculent pas le bilan dans ce fichier. Ils sont
# transmis comme métadonnées pour calcul_data.py ou pour l'audit.

MONETARY_PROFILES = {
    "carburant_financier": {
        "autorise": True,
        "categorie": "carburant_financier",
        "facteur_min": 1.20,
        "facteur_defaut": 1.42,
        "facteur_max": 1.80,
        "incertitude": 65,
        "raison": "montant carburant exploitable via prix moyen configurable",
    },
    "achats_intrants_precis": {
        "autorise": True,
        "categorie": "achats_intrants_precis",
        "facteur_min": 0.30,
        "facteur_defaut": 0.85,
        "facteur_max": 1.80,
        "incertitude": 75,
        "raison": "matériau ou intrant suffisamment identifié",
    },
    "fournitures": {
        "autorise": True,
        ...
        "facteur_max": 0.80,
        "incertitude": 80,
        "raison": "fournitures ou petit matériel catégorisé",
    },
    "location_materiel": {
        "autorise": True,
        ...
        "facteur_max": 0.70,
        "incertitude": 85,
        "raison": "location de matériel catégorisée",
    },
    "maintenance": {
        "autorise": True,
        ...
        "facteur_max": 0.60,
        "incertitude": 85,
        "raison": "entretien ou maintenance catégorisée",
    },
    "dechets_financier": {
        "autorise": True,
        "categorie": "dechets_financier",
        "facteur_min": 0.05,
        "facteur_defaut": 0.25,
        "facteur_max": 0.80,
        "incertitude": 90,
        "raison": "traitement de déchets identifié mais exprimé en euros",
    },
    "fret_financier": {
        "autorise": True,
        "categorie": "fret_financier",
        "facteur_min": 0.03,
        "facteur_defaut": 0.12,
        "facteur_max": 0.40,
        "incertitude": 90,
        "raison": "transport/fret identifié mais exprimé en euros",
    },
    "deplacements_financiers": {
        "autorise": True,
        "categorie": "deplacements_financiers",
        "facteur_min": 0.05,
        "facteur_defaut": 0.20,
        "facteur_max": 0.60,
        "incertitude": 90,
        "raison": "déplacements identifiés mais exprimés en euros",
    },
    "immobilisations": {
        "autorise": True,
        ...
        "facteur_max": 1.20,
        "incertitude": 90,
        "raison": "acquisitions immobilisées catégorisées",
    },
    "achats_non_detailles_recap": {
        "autorise": True,
        "categorie": "achats_non_detailles_recap",
        "facteur_min": 0.70,
        "facteur_defaut": 1.88,
        "facteur_max": 3.00,
        "incertitude": 95,
        "raison": "achats matériels non détaillés issus d'un récap achat bilan carbone",
    },
    "autres_services_recap": {
        "autorise": True,
        "categorie": "autres_services_recap",
        "facteur_min": 0.05,
        "facteur_defaut": 0.196,
        "facteur_max": 0.50,
        "incertitude": 90,
        "raison": "autres services issus d'un récap achat bilan carbone",
    },
    "achats_matieres_appro_agrege": {
        "autorise": True,
        "categorie": "achats_matieres_appro_agrege",
        "facteur_min": 0.15,
        "facteur_defaut": 0.50,
        "facteur_max": 1.20,
        "incertitude": 98,
        "raison": "achats matières / approvisionnements agrégés estimés avec forte incertitude",
    },
    "charges_externes_agregees": {
        "autorise": True,
        "categorie": "charges_externes_agregees",
        "facteur_min": 0.02,
        "facteur_defaut": 0.052,
        "facteur_max": 0.20,
        "incertitude": 98,
        "raison": "services / charges externes agrégés estimés avec forte incertitude",
    },
    "revue_manuelle": {
        "autorise": False,
        "categorie": "revue_manuelle",
        "facteur_min": None,
        "facteur_defaut": None,
        "facteur_max": None,
        "incertitude": 100,
        "raison": "poste financier trop large ou insuffisamment ventilé",
    },
}

BROAD_FINANCIAL_DESIGNATIONS = {
...
def monetary_profile_for_item(item: dict) -> dict:
    """Classe un montant financier sans bloquer les postes utiles.

    Principe générique :
    - les données physiques restent prioritaires ;
    - un montant en euros exploitable est calculé en estimation variable ;
    - on ne bloque que les valeurs inutiles / non sources / incohérentes.

    Les catégories très agrégées sont acceptées, mais avec une incertitude forte.
    Cela évite de sous-estimer fortement le Scope 3 quand le client fournit surtout
    une comptabilité ou un bilan fournisseurs.
    """
    role = norm(item.get("role", ""))
    designation = norm(item.get("designation", ""))

    # Récap achat préparé pour le bilan carbone : facteur plus spécifique.
    ...
    # Comptabilité agrégée : ne pas bloquer, sinon le bilan devient artificiellement bas.
    if designation in {
        "achats matieres et approvisionnements",
        "achats matières et approvisionnements",
        "achats matieres / approvisionnements",
        "achats matières / approvisionnements",
    ...
    # Postes larges mais utiles : estimation agrégée très incertaine plutôt que blocage.
    if designation in {
        "achats materiels non detailles",
        "achats matériels non détaillés",
        "sous-traitance de chantier",
        "sous traitance de chantier",
    } or role in {"achats_non_detailles", "sous_traitance"}:
        return MONETARY_PROFILES["achats_matieres_appro_agrege"]
    ...
    if designation in {
        ...
        "energie achetee",
        "énergie achetée",
    } or role in {"services", "assurances", "eau_financiere", "energie_financiere"}:
        return MONETARY_PROFILES["charges_externes_agregees"]

    # Catégories déjà fiables ou semi-précises.
    ...
    # Dernier recours : un montant financier reconnu mais non classé reste exploitable
    # comme charge externe très incertaine, au lieu d'être supprimé du bilan.
    return MONETARY_PROFILES["charges_externes_agregees"]

def apply_monetary_policy(item: dict) -> dict:
    """Ajoute les métadonnées utiles au calcul et bloque les postes trop larges.

    Les montants utiles restent en euros avec une catégorie monétaire.
    REVUE_MANUELLE ne doit servir qu'aux valeurs non exploitables ou inutiles.
    """
    ...
    item["justification"] = (
        ...
        + " | poste financier trop large : ventilation ou validation manuelle obligatoire"
    ).strip(" |")
    return item
...
def detect_role(filename: str, df: pd.DataFrame | None, raw_text: str) -> str:
    ...
    if any(k in blob for k in ["antargaz", "primagaz", "bouteille", "propane", "butane", "carburation 13 kg"]):
        return "propane"
    if any(k in blob for k in ["deplacements domicile", "déplacements domicile", "distance aller simple", "conso totale en l"]):
        return "deplacements"
...
def _classify_accounting_label(label: str, account: str) -> tuple[str, str, bool] | None:
    """Classe les charges comptables avec une logique générale.

    Le booléen retourné indique si le poste doit rester en revue manuelle.
    Règle centrale : les comptes larges restent bloqués, même si un facteur
    monétaire existe ailleurs. Les comptes précis gardent leur catégorie afin
    que calcul_data.py puisse appliquer un facteur monétaire variable.
    """
    text = norm(label)
    account = str(account)
    ...
    # Sous-traitance : trop dépendante du contenu réel du chantier.
    ...
    # Transport et déplacements : catégories identifiables mais monétaires.
    ...
    # 606 : énergie, carburant, eau et fournitures.
    ...
    # Locations, maintenance et assurances.
    ...
    # Autres services : trop large par défaut.
    if account.startswith(("618", "621", "622", "623", "626", "628")):
        return "Autres services", "services", True

    # Services bancaires : généralement hors bilan carbone automatisé.
...
def extract_edf(path: str, filename: str, scope: str, df: pd.DataFrame | None) -> list[dict]:
    ...
    # Cas particulier : fichier de prorata électricité par surface.
    # Le nombre 4900 correspond à des m², pas à des kWh consommés par l'entreprise.
    if "superficie totale" in blob and ("kwh /m2" in blob or "kwh /m²" in blob):
        return []
...
def classify_financial_by_source(filename: str) -> tuple[str, str] | None:
    """Classe une facture d'après son nom quand l'OCR est pauvre."""
    f = norm(filename).replace("\\", "/").replace("__", "/")
    ...
    if any(k in f for k in [
        ...
        return "EPI, fournitures admin. et petit matériel", "fournitures"
    ...
    if any(k in f for k in [
        ...
        return "Électricité et climatisation", "maintenance"
    ...
    if any(k in f for k in [
        ...
        return "Traitement et évacuation des déchets", "dechets_financier"
...
def classify_financial(text: str) -> tuple[str, str] | None:
    ...
    source_category = classify_financial_by_source(d)
    if source_category is not None:
        return source_category

    if any(w in d for w in FIN_REJECT_WORDS):
        return None
...
def extract_invoice_total_fallback(text: str, filename: str) -> tuple[float | None, str, str]:
    """Fallback prudent pour factures OCRisées sans Total HT propre."""
    if not text:
        return None, "", ""
    ...
    weak_words = ["total", "net", "ht", "ttc"]

    for words, label in (
        (strong_words, "montant facture probable"),
        (weak_words, "montant facture probable faible"),
    ):
        candidates: list[tuple[float, str]] = []

        for index, line in enumerate(lines):
            if not any(word in norm(line) for word in words):
                continue

            window = " ".join(lines[max(0, index - 1): min(len(lines), index + 3)])

            for value in _money_candidates(window):
                if 10 <= value <= 2_000_000 and not (1900 <= value <= 2100):
                    candidates.append((value, window[:300]))

        if candidates:
            value, proof = max(candidates, key=lambda candidate: candidate[0])
            return value, label, proof

    if classify_financial_by_source(filename) is None:
        return None, "", ""

    values = [
        (value, line[:300])
        for line in lines
        for value in _money_candidates(line)
        if 10 <= value <= 200_000 and not (1900 <= value <= 2100)
    ]

    if not values:
        return None, "", ""

    value, proof = max(values, key=lambda candidate: candidate[0])
    return value, "fallback facture par montant plausible", proof
...
def extract_invoice_financial(
    ...
    if amount is None:
        amount, basis, proof = extract_invoice_total_fallback(text, filename)

    if amount is None:
        return []

    category = classify_financial(filename + " " + text[:5000]) or classify_financial_by_source(filename)
    if category is None:
    ...
    item = make_item(
        ...
        "€",
        "moyenne",
        f"total de facture ({basis})",
    )
    item["preuve"] = proof[:500]
    item["detail_financier"] = True
    item["base_montant"] = basis
    item["facture_fallback"] = "fallback" in basis.lower() or "probable" in basis.lower()
...
def extract_recap_achat_synthese(df: pd.DataFrame | None, filename: str, scope: str) -> list[dict]:
    ...
    if autres_services > 0:
        # Autorisé uniquement ici car la source est un récap achat dédié au bilan carbone.
        # Les autres fichiers comptables vagues restent bloqués par la politique générale.
    ...
    if 1000 <= reste_materiel <= total_ht:
        item = make_item(
            filename, scope, "achats_non_detailles",
            "Achats matériels non détaillés",
            reste_materiel, "€", "faible",
            "reste du total HT après retrait services/location/entretien"
        )
        # Autorisé uniquement ici car la source est un récap achat dédié au bilan carbone.
        # Les achats non détaillés issus d'un grand livre restent bloqués ailleurs.
        out.append(item)
...
def extract_financial(
    ...
    is_invoice = (
        ...
        or "echeancier" in f
        or "échéancier" in f
        or classify_financial_by_source(filename) is not None
    )
...
def remove_invoice_details_covered_by_accounts(data: list[dict]) -> list[dict]:
    ...
    for item in data:
        ...
        role = norm(item.get("role", ""))
        purchase_roles = {"achats_intrants", "fournitures", "achats_non_detailles"}
        service_roles = {
            "services",
            "maintenance",
            "location_materiel",
        ...
        covered = (
            (covers_purchases and role in purchase_roles)
            or (covers_services and role in service_roles)
...
# ---------------------------------------------------------------------------
# Extracteurs complémentaires pour fichiers de collecte PME / HDQUALITE
# ---------------------------------------------------------------------------


def _read_csv_no_header(path: str) -> pd.DataFrame | None:
    try:
        return pd.read_csv(path, sep=";", encoding="utf-8-sig", header=None, on_bad_lines="skip")
    except Exception:
        try:
            return pd.read_csv(path, sep=",", encoding="utf-8-sig", header=None, on_bad_lines="skip")
        except Exception:
...
def _first_number_in_row(row: pd.Series, start_col: int = 0) -> float | None:
    for value in row.iloc[start_col:].values:
...
def _find_value_in_raw_csv(path: str, keywords: list[str]) -> float | None:
    raw = _read_csv_no_header(path)
    if raw is None or raw.empty:
        return None

    for _, row in raw.iterrows():
        line = norm(" ".join(str(value) for value in row.values))
        if all(keyword in line for keyword in keywords):
            values = [
                to_float(value)
                for value in row.iloc[1:].values
            ]
...
def _find_company_surface_near(path: str) -> float | None:
    folder = Path(path).parent

    for candidate in folder.glob("*Données*.csv"):
        raw = _read_csv_no_header(str(candidate))
        if raw is None or raw.empty:
            continue

        for _, row in raw.iterrows():
            line = norm(" ".join(str(value) for value in row.values))
            if ("batiment" in line or "bâtiment" in line) and ("m2" in line or "m²" in line):
                value = _first_number_in_row(row, 1)
                if value is not None and 1 <= value <= 100_000:
                    return value

    return None


def extract_electricity_prorata_surface(
    path: str,
    filename: str,
    scope: str,
    df: pd.DataFrame | None,
) -> list[dict]:
    """Extrait une électricité proratisée quand un fichier donne kWh total + surface."""
    blob = norm(filename + " " + table_text(df, 5000))

    if "kwh /m2" not in blob and "kwh / m2" not in blob and "superficie totale" not in blob:
        return []

    total_kwh = _find_value_in_raw_csv(path, ["conso totale"])
    total_surface = _find_value_in_raw_csv(path, ["superficie totale"])
    company_surface = _find_company_surface_near(path)

    if not total_kwh or not total_surface or not company_surface:
        return []

    if not (100 <= total_kwh <= 100_000_000 and 1 <= total_surface <= 1_000_000):
        return []

    prorated_kwh = total_kwh / total_surface * company_surface

    if not (1 <= prorated_kwh <= total_kwh):
        return []
    ...
    item = make_item(
        ...
        "kWh",
        "moyenne",
        f"électricité proratisée : {total_kwh:.0f} kWh / {total_surface:.0f} m² × {company_surface:.0f} m²",
    )
    item["preuve"] = f"{total_kwh:.0f} kWh ; {total_surface:.0f} m² total ; {company_surface:.0f} m² entreprise"

    print(f"  [DIRECT ELEC PRORATA] {prorated_kwh:.2f} kWh")
    return [item]


def extract_synthese_donnees_collecte(
    path: str,
    filename: str,
    scope: str,
) -> list[dict]:
    """Extrait les données sources d'une feuille de collecte, sans lire Recap CO2e."""
    f = norm(filename)

    stem_raw = Path(filename).stem.lower()
    is_donnees_sheet = stem_raw.endswith("_données") or stem_raw.endswith("_donnees")

    if "recap co2" in f or "recap co2e" in f:
        return []

    if not is_donnees_sheet:
        return []
    ...
    output: list[dict] = []
    full_text = norm(raw.fillna("").astype(str).to_string())

    if "r404a" in full_text and ("pompe a chaleur" in full_text or "clim reversible" in full_text):
        puissance_kw = None

        for _, row in raw.iterrows():
            line = norm(" ".join(str(value) for value in row.values))
            if "puissance" in line and "kw" in line:
                puissance_kw = _first_number_in_row(row, 1)
                break

        if puissance_kw is not None and 0 < puissance_kw <= 500:
            kg_co2e = puissance_kw * 141.84
            item = make_item(
                filename,
                "SCOPE_1",
                "combustible_direct",
                "Gaz PAC - fluide frigorigène R404A",
                kg_co2e,
                "kgCO2e",
                "faible",
                "estimation contrôlée depuis puissance PAC et fluide R404A",
            )
            item["preuve"] = f"R404A ; puissance PAC = {puissance_kw:g} kW"
            output.append(item)
    ...
    service_rows = {
        ...
        "evenementiel": "Evènementiel",
        "évènementiel": "Evènementiel",
        "assurance": "Assurance",
    }
    ...
    for _, row in raw.iterrows():
        ...
        matched_label = None
        for key, label in service_rows.items():
            if key in line:
                matched_label = label
                break
        ...
        value = _first_number_in_row(row, 1)
        if value is None or not (1 <= value <= 10_000_000):
            continue
        ...
        output.append(
            make_item(
                ...
                "€",
                "moyenne",
                f"montant service extrait de la feuille Données : {matched_label}",
            )
        )
    ...
    for _, row in raw.iterrows():
        line = norm(" ".join(str(value) for value in row.values))
        if ("batiment" in line or "bâtiment" in line) and ("m2" in line or "m²" in line):
            surface = _first_number_in_row(row, 1)
            if surface is not None and 1 <= surface <= 100_000:
                kg_co2e = surface * 21.67
                item = make_item(
                    filename,
                    "SCOPE_3",
                    "immobilisations",
                    ...
                    "kgCO2e",
                    "faible",
                    "estimation annuelle depuis surface bâtiment collectée",
                )
                item["preuve"] = f"surface bâtiment = {surface:g} m²"
                output.append(item)
            break

    if output:
        print(f"  [DIRECT DONNÉES COLLECTE] {len(output)} donnée(s)")

    return output


def extract_deplacements_domicile_travail(
    path: str,
    filename: str,
    scope: str,
    df: pd.DataFrame | None,
) -> list[dict]:
    """Extrait les litres depuis un tableau domicile-travail."""
    blob = norm(filename + " " + table_text(df, 5000))

    if not any(k in blob for k in ["deplacements domicile", "déplacements domicile", "distance aller simple"]):
        return []

    if df is None or df.empty:
        return []

    fuel_col = next((col for col in df.columns if "carburant" in norm(col) and "motorisation" in norm(col)), None)
    liters_col = next((col for col in df.columns if "conso totale" in norm(col) and "l" in norm(col)), None)

    if fuel_col is None or liters_col is None:
        return []

    totals = {
        "gazole": 0.0,
        "essence": 0.0,
        "vehicule_fonction_essence": 0.0,
    }

    for _, row in df.iterrows():
        fuel = norm(row.get(fuel_col, ""))
        liters = to_float(row.get(liters_col))

        if liters is None or not (1 <= liters <= 100_000):
            continue

        row_blob = norm(" ".join(str(value) for value in row.values))
        if ("total" in row_blob or "delta" in row_blob or "j sur site" in row_blob) and not fuel:
            continue

        if "voiture de fonction" in row_blob and "essence" in fuel:
            totals["vehicule_fonction_essence"] += liters
        elif "gazole" in fuel or "diesel" in fuel:
            totals["gazole"] += liters
        elif "essence" in fuel:
            totals["essence"] += liters

    output: list[dict] = []

    if totals["gazole"] > 0:
        output.append(make_item(filename, "SCOPE_3", "deplacements", "Gazole routier - déplacements domicile-travail", totals["gazole"], "L", "moyenne", "somme des litres gazole du tableau domicile-travail"))

    if totals["essence"] > 0:
        output.append(make_item(filename, "SCOPE_3", "deplacements", "Essence E10 - déplacements domicile-travail", totals["essence"], "L", "moyenne", "somme des litres essence du tableau domicile-travail"))

    if totals["vehicule_fonction_essence"] > 0:
        output.append(make_item(filename, "SCOPE_3", "deplacements", "Essence E10 - véhicule de fonction", totals["vehicule_fonction_essence"], "L", "moyenne", "litres essence du véhicule de fonction dans le tableau déplacements"))

    if output:
        print(f"  [DIRECT DÉPLACEMENTS] {len(output)} carburant(s)")

    return output


def extract_parc_informatique(
    path: str,
    filename: str,
    scope: str,
    df: pd.DataFrame | None,
) -> list[dict]:
    """Extrait une estimation d'immobilisations depuis un inventaire informatique."""
    blob = norm(filename + " " + table_text(df, 8000))

    if "parc informatique" not in blob and "pc portable" not in blob:
        return []

    if df is None or df.empty:
        return []

    counts = {"pc_portable": 0, "ecran": 0, "imprimante": 0, "telephone": 0, "nas": 0}

    for _, row in df.iterrows():
        line = norm(" ".join(str(value) for value in row.values))
        if "actif" not in line:
            continue
        if "pc portable" in line:
            counts["pc_portable"] += 1
        elif "imprimante" in line:
            counts["imprimante"] += 1
        elif "telephone portable" in line or "téléphone portable" in line:
            counts["telephone"] += 1
        elif "ecran" in line or "écran" in line:
            counts["ecran"] += 1
        elif "nas" in line:
            counts["nas"] += 1

    annual_kg = (
        counts["pc_portable"] * 156 / 4
        + counts["ecran"] * 80 / 6
        + counts["imprimante"] * 80 / 5
        + counts["telephone"] * 40 / 3
        + counts["nas"] * 100 / 5
    )
    ...
    item = make_item(filename, "SCOPE_3", "immobilisations", "Immobilisations informatiques", annual_kg, "kgCO2e", "faible", "estimation annuelle depuis inventaire parc informatique")
    item["preuve"] = ", ".join(f"{key}={value}" for key, value in counts.items() if value)

    print(f"  [DIRECT PARC INFO] {annual_kg:.2f} kgCO2e")
    return [item]


def detail_invoice_group(source: str) -> str:
    f = str(source).replace("\\", "/").replace("__", "/")
    parts = [part.strip() for part in f.split("/") if part.strip()]
    for part in parts:
        if "facture" in norm(part):
            return part[:80]
    return source_family(source) or Path(str(source)).stem[:80]


def aggregate_invoice_details(data: list[dict]) -> list[dict]:
    """Agrège plusieurs factures individuelles d'un même groupe."""
    groups: dict[tuple, list[dict]] = defaultdict(list)
    passthrough: list[dict] = []
    for item in data:
        if item.get("detail_financier") and not item.get("agregat_detail_financier"):
            key = (item.get("scope", ""), norm(item.get("role", "")), norm(item.get("designation", "")), item.get("unite", ""), detail_invoice_group(item.get("source", "")))
    ...
    for key, items in groups.items():
        if len(items) < 2:
            ...
            continue
        first = dict(items[0])
        total = sum(float(item.get("quantite", 0) or 0) for item in items)
        first["quantite"] = round(total, 4)
        first["source"] = f"{key[-1]} — agrégé {len(items)} facture(s)"
        first["source_family"] = source_family(first["source"])
        first["justification"] = f"agrégation de {len(items)} factures individuelles ({first.get('designation')})"
        first["preuve"] = " | ".join(str(item.get("source", ""))[:80] for item in items[:12])
        first["agregat_detail_financier"] = True
        passthrough.append(first)
        print(f"  [AGRÉGATION FACTURES] {first['designation']} : {len(items)} factures -> {total:.2f} €")
    return passthrough
...
def extract_one(path: str, scope: str, target_year: int | None = None) -> list[dict]:
    ...
    standard_extractors = (
        ...
        lambda: extract_synthese_donnees_collecte(path, filename, scope),
        lambda: extract_electricity_prorata_surface(path, filename, scope, df),
        lambda: extract_deplacements_domicile_travail(path, filename, scope, df),
        lambda: extract_parc_informatique(path, filename, scope, df),
...
def lancer_le_bilan(base_path: str = ".") -> tuple[list[dict], int]:
    ...
    all_data = aggregate_invoice_details(all_data)
